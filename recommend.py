...
List Collaborators: Sophie Hsu, Maryan Le
...
def find_closest(location, centroids):
    ...
    Returns the closest [x,y] coordinate in centroids to the location given
...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    # take one centroid, can find the closest with one resturant
    # take the second centroid, find the cloest with the one resturant, 
    # compare and then group into one cluster for centroid A or cluster for centroid B
    pairs = [[find_closest(restaurant_location(r), centroids), r] for r in restaurants]
    # pairs have to be [centroid, resturant] the centroid that is the closest to the resturant
    # all the common centroids are taken in, and the group_by_key 
    # gives you the values which are the resturants in this case
...
def find_centroid(cluster):
    ...
    avg_lat = mean(latitudes)
    avg_long = mean(longitudes)
...
def k_means(restaurants, k, max_updates=100):
    ...
    while previous_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        # clusters gives you groups of resturants that are near each other
        clusters = group_by_centroid(restaurants, previous_centroids)

        centroids = [find_centroid(cluster) for cluster in clusters]
        
        '''
        this is the same as the one line above
        new_centroid = []

        for cluster in clusters:
            new_centroid.append(find_centroid(cluster))
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # Sxx is how much x varies
    # Syy is how much y varies
    # Sxy how much x and y varies together
    ...
    a = y_mean - b * x_mean
    # R^2 = 1 perfect fit
    # R^2 = 0.0 feature doesn't explain anything
    r_squared = (Sxy ** 2) / (Syy * Sxx)
    # END Question 7
...
# What feature gives the best prediction for the user's ratings,
# What feature best explains how the user gives its ratings

def best_predictor(user, restaurants, feature_fns):
    # feature_fns are all the possible feature functions
    # some features are price, average rating, num ratings, etc
    ...
    # this needs to be used as an argument in user_reviewed_restaurants
    # because your trying to find the best prediction based on the user's ratings
    # or in other words the resturants the user's rated
    reviewed = user_reviewed_restaurants(user, restaurants)
    
    # BEGIN Question 8
    # find_predictor returns (predictor, rsquared) each is its own element in the list
    pairs = [find_predictor(user, reviewed, feature) for feature in feature_fns]

    # This gives you a pair, the max pair is found by comparing the rsquared values
    pair_w_highest_rsquared = max(pairs, key = lambda pair : pair[1])

    # this returns the predictor
    return pair_w_highest_rsquared[0]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            score = (user_score(user, name))
        else:
            score = (predictor(restaurant))
...
def search(query, restaurants):
    ...
    for restaurant in restaurants:
        if query in restaurant_categories(restaurant):
            restaurants_w_query.append(restaurant)
    
    return restaurants_w_query
