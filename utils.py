...
def map_and_filter(s, map_fn, filter_fn):
    ...
    """
    # BEGIN Question 0
    # check the condition, if it's true, apply the function and return the new list
...
# * before sequences allows you to take any number of arguments 
def zip(*sequences):
    ...
    ['c', 3, 'mi']
    """
    # we are overriding python's current zip function
    # so instead we would have to use _zip to get python's zip function instead of 
    # our built-in zip function we just made 
    # _zip is a python function that takes in any number of 
